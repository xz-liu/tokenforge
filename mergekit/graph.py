...
class Task(ABC, BaseModel, Generic[ValueT], frozen=True):
    ...
    # Pydantic v2 tries to generate schemas for all field types. Many tasks carry
    # tensors (e.g., `torch.Tensor`) and other non-serializable runtime objects;
    # allow arbitrary types so schema generation doesn't fail at import time.
